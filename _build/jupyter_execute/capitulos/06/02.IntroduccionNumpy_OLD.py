...
# 
# Un vector unidimensional es una colección ordenada de números a los que se puede acceder mediante el índice correspondiente (empezando a contar en cero). Por defecto, los vectores en **Numpy** son vectores fila.
...
# Lo que no es posible, por ejemplo, es añadir una cadena de texto (de tipo `string`) a un objeto `numpy.ndarray`, ya que todos los elementos del vector deben ser del mismo tipo (o de tipos que admitan una conversión) y deben tener también el mismo tamaño. Para comprobar el tamaño de un vector, se puede usar la función `len`:
...
print(u + v) # Implícitamente se hace una conversión de tipo de int64 a float64
print(u + w) # ERROR: Los vectores non tienen el mismo tamaño!


# Una forma más específica de comprobar la dimensión del vector u es usar `u.shape`, que nos devuelve una tupla con las dimensiones del vector:
...
# Podemos cambiar las componentes de un vector utilizando la indexación,
...
# 
# Evidentemente, existen otras maneras de crear vectores, como, por ejemplo, el uso de la función `ones` para crear un vector que contiene solamente 'unos':
...
# o de la función `random.rand` para un vector con valores aleatorios:
...
# o también podemos crear un vector de números de tipo `numpy.array` a partir de una lista Python de números:
...
# Existen otros dos métodos para crear vectores de números que nos serán de utilidad a lo largo del curso (y en particular cando tegamos que pintar funciones con **Matplotlib** en una o en varias variables):
# - `numpy.arange` 
# - `numpy.linspace`
# 
...
# Podemos comprobar que el número $6$ no está incluido ya que el rango de valores comienza en $0$ y el vector solamente posee seis elementos. Sin embargo, podemos cambiar el valor numérico en el que comienza el vector: 
...
# Esta función `linspace` junto con la función `meshgrid` se usará de forma habitual para pintar funciones de una y varias variables a lo largo de este curso.

# ## Funciones y aritmética sobre vectores
# 
# Los vectores en **NumPy** soportan las operaciones aritméticas básicas (productos, sumas, restas, etc.):
...
# También se pueden aplicar las funciones de cálculo usuales a cada una de sus componentes:
...
# El código aterior calcula el seno de cada componente del vector `a`. Debemos remarcar que la función que se está utilizando es `np.sin`, que depende directamente del módulo **Numpy**. El uso de cualquier otra implementación de la función en otros módulos (por ejemplo, en el módulo **Math**), podría dar lugar a error.
...
# Si lo que se quiere es trocear un vector desde el principio o hasta el final del mismo, hay que utilizar la sintaxis de índices con '`:`'
