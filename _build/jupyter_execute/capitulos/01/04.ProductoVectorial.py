...
# 6. $\mathbf{u}\cdot\left( \mathbf{v} \times \mathbf{w}  \right) = \left( \mathbf{u} \times \mathbf{v}  \right) \cdot \mathbf{w}$.
# 
# ````
# 
...
# 3. $ \mathbf{u}\times\mathbf{v} = \mathbf{0}$ si y sólo si $\mathbf{u}$ y $\mathbf{v}$ son paralelos (es decir, si $\exists c\in\mathbb{R}$ tal que $\mathbf{v} = c \mathbf{u}$).
